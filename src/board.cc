...
std::string Board::getFen() { return this->chessRules.ForsythPublish(); }
...
std::array<boolBoard, 19> Board::boardToInput() {
  ...
  // std::cout << "Time to convert board to input: " << duration.count() << "us"
  // << std::endl;

  return input;
}

std::map<thc::Move, float>
Board::outputProbsToMoves(std::array<floatBoard, 73> &outputProbs,
...
thc::ChessRules Board::getChessRules() { return this->chessRules; }